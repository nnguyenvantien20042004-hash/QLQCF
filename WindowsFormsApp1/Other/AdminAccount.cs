...
namespace WindowsFormsApp1
{
    public partial class AdminAccount : Form
    {
        ...
        {
            // 1. Bắt lỗi: chưa chọn bàn
            if (string.IsNullOrWhiteSpace(txtIdTable.Text))
            {
                MessageBox.Show("Hãy chọn bàn cần sửa ở danh sách bên trái!",
                                "Thông báo",
            ...
            // 2. Bắt lỗi: tên bàn mới rỗng
            if (string.IsNullOrWhiteSpace(txtNewTable.Text))
            {
                MessageBox.Show("Tên bàn mới không được để trống!",
                                "Thông báo",
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Warning);
                return;
            ...
            string newName = txtNewTable.Text.Trim();

            // 3. (tuỳ chọn) Kiểm tra trùng tên
            int exists = dataProvider.Instance.excuteExist(
                "SELECT * FROM dbo.TABLESTORE WHERE name = N'" + newName + "'");
            if (exists > 0)
            {
                MessageBox.Show("Đã tồn tại bàn tên \"" + newName + "\" rồi!",
                                "Thông báo",
                                MessageBoxButtons.OK,
            ...
            // 4. Câu UPDATE theo idTABLE, dùng NonQuerry
            string querry = "UPDATE dbo.TABLESTORE SET name = @newName WHERE idTABLE = @id";
            dataProvider.Instance.excuteNonQuerry(querry, new object[] { newName, idTable });

            MessageBox.Show("Đổi tên bàn thành công!",
                            "Thông báo",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Information);

            // 5. Reload lại giao diện/table list
            load_Table();            // nếu cậu có hàm này public
                                     // hoặc vẫn dùng kiểu cũ:
                                     // btn_Click(sender, e);  // mở lại AdminAccount để refresh
        }

        private void button7_Click(object sender, EventArgs e)
        {
            // Bỏ check bill luôn nếu DB không có status

            string query = "DELETE FROM dbo.TABLESTORE WHERE idTABLE = @id";
            dataProvider.Instance.excuteNonQuerry(query, new object[] { idTable });
        ...
        {
            if (dtgvBill.DataSource is DataTable dt)
            {
                dt.DefaultView.Sort = "[Số Lượng] DESC";
