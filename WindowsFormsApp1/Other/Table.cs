...
namespace WindowsFormsApp1
{
    public partial class Table : Form
    {
        ...
        private float originalTotal = 0;   // Tổng tiền trước giảm
        private float discountedTotal = 0; // Tổng tiền sau giảm
        ...
        public void showBill(int tableID)
        {
            ...
            if (dataProvider.Instance.excuteExist("SELECT * FROM TABLESTORE WHERE idTABLE = " + idTable + " AND isGood = 0") > 0)
            {
                ...
                originalTotal = TotalPrice;   // lưu tổng tiền gốc
                discountedTotal = TotalPrice;   // mặc định chưa giảm

                TongTien.Text = discountedTotal.ToString("c", culture);
            }
            else
            {
                originalTotal = 0;
        ...
        {
            AboutSoftware f = new AboutSoftware();
            f.ShowDialog();
        }

        private void hướngDẫnSửDụngToolStripMenuItem_Click(object sender, EventArgs e)
        {
            UserGuide f = new UserGuide();
            f.ShowDialog();
        }

        private void btnVouncher_Click(object sender, EventArgs e)
        {
            // Không có bill thì khỏi giảm
            if (originalTotal <= 0)
            {
                MessageBox.Show("Chưa có hóa đơn nào để áp dụng voucher!",
                                "Thông báo",
            ...
            if (string.IsNullOrWhiteSpace(input))
                return; // bấm Cancel hoặc bỏ trống thì thoát

            if (!float.TryParse(input, out float percent) || percent < 0 || percent > 100)
            {
            ...
            discountedTotal = originalTotal * (100 - percent) / 100f;

            CultureInfo culture = new CultureInfo("vi-vn");
            TongTien.Text = discountedTotal.ToString("c", culture);

            MessageBox.Show(
                $"Đã áp dụng giảm {percent}%.\nTổng tiền sau giảm: {discountedTotal.ToString("c", culture)}",
                "Voucher",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information
            );
